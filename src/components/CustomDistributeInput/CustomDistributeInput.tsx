...
export default function CustomDistributeInput({
  idLocal,
  nombreDeLocal,
  inputName,
  idProducto,
  ...
  useEffect(() => {
    const getStock = async () => {
      ...
      setInputVal(res.productStock![0].cantidadLocal);
      setIsLoading(false);
    };
    getStock();
  }, []);
  ...
  return (
    <div
      className='min-w-[300px] p-4 bg-purple-200 shadow-lg overflow-hidden rounded-xl'
      key={idLocal}
    >
      <h4 className='text-violet-700 font-semibold '>{nombreDeLocal}</h4>
      ...
      <div>
        {isLoading ? (
          <div role='status'>
            <svg
              aria-hidden='true'
              className='m-auto w-8 h-8 text-gray-100 animate-spin fill-violet-600'
              viewBox='0 0 100 101'
              fill='none'
